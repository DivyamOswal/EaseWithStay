  ...
  return (
    <div className="bg-[#E6E6E6] px-6 py-4">
      <div className="max-w-[1200px] w-full mx-auto overflow-hidden">
        ...
        >
          {familyFriendlys.map((familyFriendly) => (
            <SwiperSlide key={familyFriendly.id} className="pt-4">
              {/* Link (navigate to the hotel details page) */}
              <Link
                to={`/bungalows/${familyFriendly.id}`}
                state={{ source: "familyFriendly" }} // ✅ pass category name
              >
                <div className="rounded-lg shadow hover:shadow-lg transition bg-white">
                  <div>
                    <img
                      src={familyFriendly.image}
                      alt={familyFriendly.name}
